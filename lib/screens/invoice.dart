...
class _InvoiceScreenState extends State<InvoiceScreen>
  ...
  void addMedicine() {
    ...
    // Auto: use stock rate if available, otherwise manual input
    double unitPrice;
    if (currentPerTabletRate > 0 && !isManualRate) {
      unitPrice = currentPerTabletRate;
    } else {
    ...
    setState(() {
      cart.add({
        "name": name,
        "qty": qty,
        "unitPrice": unitPrice, // keep full value
        "price": totalPrice, // keep full value
      });
      ...
      // Update total field (2 decimals for UI)
      totalController.text = grandTotal.toStringAsFixed(2);

      _clearInputs();
      nameFocus.requestFocus();
      isManualRate = false; // reset after add
    });
  }
                              ...
                              Expanded(
                                ...
                                child: _keyboardTextField(
                                  ...
                                  onChanged: (value) {
                                    ...
                                    // Use stock rate if available and not in manual mode
                                    final double unitPrice = (!isManualRate &&
                                            currentPerTabletRate > 0)
                                        ? currentPerTabletRate
                                        : double.tryParse(
                                                priceController.text.trim()) ??
                                            0.0;
                              ...
                              Expanded(
                                ...
                                child: _keyboardTextField(
                                  ...
                                  onChanged: (value) {
                                    ...
                                    setState(() {
                                      isManualRate =
                                          true; // force manual mode once user edits
                                      if (qty > 0 && unitPrice > 0) {
                                        final total = qty * unitPrice;
                                        totalController.text =
                                            total.toStringAsFixed(
